...
def update_load_patch(content, load_id):
    """ Updates a load with PATCH """
    load_key = client.key(constants.loads, int(load_id))
    load = client.get(key=load_key)
    if load:
        for key in content:
        ...
        return {"Error": "No load with this load id exists"}, 404


def update_load_put(content, load_id):
    """ Updates a load with PUT method """
    load_key = client.key(constants.loads, int(load_id))
    load = client.get(key=load_key)
    if load:
        try:
